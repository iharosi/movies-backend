...
let TMDbClient = function(key) {
    ...
    this.log = function(options) {
        let str = ['\x1b[35mAPI call\x1b[0m'];
        if (options.method === 'GET') {
            str.push(`\x1b[32m${options.method}\x1b[0m`);
        } else if (options.method === 'POST') {
            str.push(`\x1b[36m${options.method}\x1b[0m`);
        } else {
            str.push(`\x1b[1m${options.method}\x1b[0m`);
        }
        if (options.url) {
            str.push(`\x1b[0m${options.url.replace(this.basePath, '')}\x1b[0m`);
        }
        if (options.qs) {
            str.push(`\x1b[2m${JSON.stringify(options.qs)}\x1b[0m`);
        }
        if (options.body && Object.getOwnPropertyNames(options.body).length) {
            str.push(`– \x1b[2m${JSON.stringify(options.body)}\x1b[0m`);
        }
